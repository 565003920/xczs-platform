"""v2.0 unified router — all new endpoints."""
...
def api_student_profile(student_id: int, db: Session = Depends(get_db)):
    try:
...
def api_class_students(class_id: int, db: Session = Depends(get_db)):
    return get_class_students_profile(db, class_id)


# ---- Migration ----
@router.get("/migration/recommend")
def api_migration_recommend(target_class_id: int, db: Session = Depends(get_db)):
    try:
...
def api_lesson_plan(body: dict, db: Session = Depends(get_db)):
    try:
        plan = generate_lesson_plan(db, body["class_id"], body.get("lesson_topic", ""), body.get("duration", 50))
        record_lineage(f"lesson_{body['class_id']}_{body.get('lesson_topic','')}", "lesson_plan",
                       [{"source": "class_profile", "class_id": body["class_id"]}],
                       {"type": "lesson_plan", "class_id": body["class_id"]})
        return plan
    except ValueError as e:
        raise HTTPException(404, str(e))
...
def api_reflection(body: dict, db: Session = Depends(get_db)):
    try:
        ...
        record_lineage(f"reflection_{body['class_id']}", "reflection",
                       ...
                       {"type": "reflection", "class_id": body["class_id"]})
        return ref
    except ValueError as e:
        raise HTTPException(404, str(e))
...
def api_lineage_detail(analysis_id: str):
    ...
    if not r:
        raise HTTPException(404, "血缘记录不存在")
    return r
